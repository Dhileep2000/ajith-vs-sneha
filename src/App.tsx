...
import { EntranceSequence } from './components/entrance/EntranceSequence';
...
export const App: React.FC = () => {
  ...
  // Entrance flow: 'sequence' (108 frames) -> 'wedding'
  const [flowStep, setFlowStep] = useState<'sequence' | 'wedding'>('sequence');
  ...
  return (
    <div className="w-full min-h-screen bg-[#F6F3EB] text-[#2B1113] relative overflow-x-hidden selection:bg-[#5A121D] selection:text-[#F6F3EB]">
      ...
      {/* 1. Cinematic 108-Image Scroll Entrance (shown exclusively until 108th image finishes) */}
      {flowStep === 'sequence' && (
        <EntranceSequence
          onComplete={() => {
            window.scrollTo(0, 0);
      ...
      {/* 5. The Wedding Day Section & Full Website - Revealed ONLY after 108th image completes */}
      {flowStep === 'wedding' && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          ...
          className="w-full"
        >
          {/* Responsive Full-Width Top Navigation */}
